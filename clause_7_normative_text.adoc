...
Note that no additional relationships or foreign keys are necessary as the feature attributes are needed in the GeoPackage as the same attributes from the ShapeFile (CEAI,GEAI, and VEAI as shown in Figure 1) are used to link the features to the extended attributes in the GeoPackage file.

After each feature ShapeFile has been processed, the conversion is complete. Note that none of the files associated ShapeFile or DBF files are removed from the CDB.
